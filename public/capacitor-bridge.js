(function(){
  ...
  wait(function(){
    var F=Capacitor.Plugins.Filesystem,S=Capacitor.Plugins.Share,P=Capacitor.Plugins.Preferences;
    if(!F)return;

    // === 1. اعتراض document.write (يمنع عرض JSON على الشاشة) ===
    ...
    function saveFile(b64,name){
      F.writeFile({path:'Download/PatternBoard/'+name,data:b64,directory:'EXTERNAL_STORAGE',recursive:true})
        ...
        .catch(function(e){
          alert('❌ فشل التصدير: '+e.message+'\n\nجربي التصدير مرة أخرى.');
        });
    }

    // === 6. تخزين تلقائي (localStorage backup) ===
    if(P){
      var origSet=localStorage.setItem;
      localStorage.setItem=function(k,v){
        try{P.set({key:k,value:String(v)});}catch(e){}
        return origSet.call(localStorage,k,v);
      };
    }

    console.log('[Bridge] Capacitor bridge active');
  },50);
})();
